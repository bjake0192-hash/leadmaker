...
export class ContactFinder {
  ...
  public extractContacts(lead: Lead): Lead {
    ...
    const foundEmails: string[] = lead.crawledContent.match(emailRegex) || [];
    
    // Filter out common image extensions masquerading as emails (e.g. file@2x.png)
    const validEmails = foundEmails.filter((e: string) => !e.endsWith('.png') && !e.endsWith('.jpg') && !e.endsWith('.jpeg'));
    lead.emails = Array.from(new Set(validEmails.map((e: string) => e.toLowerCase())));

    // 2. Extract Phones (basic regex for international/US/UK formats)
    const phoneRegex = /(?:\+?\d{1,3}[\s-]?)?\(?\d{3}\)?[\s-]?\d{3}[\s-]?\d{4}/g;
    const foundPhones: string[] = lead.crawledContent.match(phoneRegex) || [];
    ...
    const foundSocials: string[] = lead.crawledContent.match(socialsRegex) || [];
    
    foundSocials.forEach((url: string) => {
